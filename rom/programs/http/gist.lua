...
function put(files, description, id, interactive)
    ...
    local handle
    if id then
        if http.patch ~= nil then handle = http.patch("https://api.github.com/gists/" .. id, textutils.serializeJSON(data):gsub("\n", "n"), headers)
        elseif http.websocket ~= nil then handle = http.post{ url = "https://api.github.com/gists/" .. id, body = textutils.serializeJSON(data):gsub("\n", "n"), headers = headers, method = "PATCH" }
        else if interactive then print("Failed: This version of ComputerCraft doesn't support the PATCH method. Update to CC: Tweaked or a compatible emulator (CraftOS-PC, CCEmuX) to use 'edit'.") end return nil end
...
function delete(id, interactive)
    ...
    else if interactive then print("Failed: This version of ComputerCraft doesn't support the DELETE method. Update to CC: Tweaked or a compatible emulator (CraftOS-PC, CCEmuX) to use 'edit'.") end return false end
