...
function create( parent, nX, nY, nWidth, nHeight, bStartVisible )
    ...
    local window = setmetatable({}, {__index = parent})
    ...
    function window.getLine(line)
        ...
        if line < 1 or line > nHeight then error("Line is out of range.", 2) end
    ...
    function window.reposition( nNewX, nNewY, nNewWidth, nNewHeight, tNewParent )
        ...
        if (nNewWidth ~= nil or nNewHeight ~= nil) and type( nNewWidth ) ~= "number" then error( "bad argument #3 (expected number, got " .. type( nNewWidth ) .. ")", 2 ) end
        if (nNewWidth ~= nil or nNewHeight ~= nil) and type( nNewHeight ) ~= "number" then error( "bad argument #4 (expected number, got " .. type( nNewHeight ) .. ")", 2 ) end
